...
class Qwen3Model(nn.Module):
    ...
    def forward(self, in_idx, targets=None, mask=None):
        """
        Forward pass compatible with GPT-style training loop:
        - Always applies a causal mask internally for attention
        - Optional `mask` can be used to mask logits (e.g., padding positions)
        - Returns (logits, loss) where loss is computed if targets are provided
        """
        ...
        # Causal mask for attention (2D: seq x seq)
        ...
        for block in self.trf_blocks:
            x = block(x, attn_mask, self.cos, self.sin)

        x = self.final_norm(x)
        full_logits = self.out_head(x.to(self.cfg["dtype"]))

        if targets is not None:
            logits = full_logits
            if mask is not None:
                # Expect mask broadcastable to logits: [b, t] or [b, t, 1]
                if mask.dim() == 2:
            ...
            # Inference path: only return logits for the last position
